...
def pitching_dashboard(df: pd.DataFrame, stats: list, pitcher_name: str, team: str, season: int, data_path: str):
    ...
    plt.tight_layout()
    filename = f"./KCL/Cards/7-13/{pitcher_name.replace(' ', '')}_pitching_dashboard.png"
    os.makedirs(os.path.dirname(filename), exist_ok=True)
    plt.savefig(filename, bbox_inches='tight', dpi=300)
    plt.close()

# Main script with updated for loop
data_path = 'KCL/Data/7-13.csv'
...
df = df[df['Pitcher'].isin(['Carter Sellers','Teagan Disharoom'])]
