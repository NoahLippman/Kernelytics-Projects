...
output:
  pdf_document:
    ...
    keep_tex: false
    output_file: C:/Users/isu_mvquirk_admin/Documents/GitHub/Kernelytics-Projects/KCL/output.pdf
  html_document:
    df_print: paged
geometry: landscape
header-includes:
...
df <- df %>%
  filter(
    PitcherTeam == 'Kcl groundslouths 25' | PitcherTeam == 'Kcl groundsloths 25'
  )
...
df <- df %>%
  mutate(
    ...
    GBindicator = ifelse(TaggedHitType=='GroundBall',1,0),  # Fixed from HitType
...
\begin{center}
\textbf{\Large Pitching Leaders}
\end{center}
\vspace{-1cm}
% \includegraphics[width = 1cm, height = 1cm]{Chant Hat Logo} % Uncomment and update path if logo available
\vspace{-0.8cm}
\hfill
% \includegraphics[width = 2.5cm, height = 1.5cm]{Own It Bronze} % Uncomment and update path if logo available
\begin{center}
\end{center}
...
summary_data %>%
  arrange(desc(`Avg FB Velo`)) %>%
  kable(format = "latex", linesep = "", label = "Leaderboard") %>%
  kable_styling(latex_options = c('scale_down', "hold_position")) %>%
  column_spec(1, border_left = TRUE, bold=TRUE, color='black') %>%
  column_spec(3, bold = TRUE, background = summary_data2$Velocity_Color) %>%
  column_spec(4, bold = TRUE, background = summary_data2$MaxColor) %>%
  column_spec(5, bold = TRUE, background = summary_data2$StrikePerColor) %>%
  column_spec(6, bold = TRUE, background = summary_data2$FBStrkColor) %>%
  column_spec(7, bold = TRUE, background = summary_data2$OSStrikeCol) %>%
  column_spec(8, bold = TRUE, background = summary_data2$EPAcol) %>%
  column_spec(9, bold = TRUE, background = summary_data2$FPScolor) %>%
  column_spec(10, bold = TRUE, background = summary_data2$Whiffcolor) %>%
  column_spec(11, bold = TRUE, background = summary_data2$zwhiffcolor) %>%
  column_spec(12, bold = TRUE, background = summary_data2$chasecol) %>%
  column_spec(13, bold = TRUE, background = summary_data2$Kcolor) %>%
  column_spec(14, bold = TRUE, background = summary_data2$BBcolor) %>%
  column_spec(15, bold = TRUE, background = summary_data2$AVGcolor) %>%
  column_spec(16, bold = TRUE, background = summary_data2$OPScol) %>%
  column_spec(17, bold = TRUE, background = summary_data2$HHcol) %>%
  column_spec(18, bold = TRUE, background = summary_data2$GBcol) %>%
